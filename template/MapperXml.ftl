    ...
    <select id="query" resultType="${package_name}.${block}.dto.${table_name?cap_first}Dto">
        ...
        <where>
            <if test="query != null and query != ''">
                <#if (table_name?substring(table_name?length-4,table_name?length) = 'Dict') ><#--如果是字典Dict结尾-->
                and ( t.itemCode like '%${r'${query}'}%' or t.itemName like '%${r'${query}'}%' or t.itemType like '%${r'${query}'}%' or t.itemPy like '%${r'${query}'}%' or t.itemWb like '%${r'${query}'}%'<#list 0..(field!?size-1) as i><#if field[i].name?uncap_first != 'id' && field[i].name?uncap_first != 'itemCode' && field[i].name?uncap_first != 'itemName' && field[i].name?uncap_first != 'itemType' && field[i].name?uncap_first != 'itemPy' && field[i].name?uncap_first != 'itemWb'> or t.${field[i].name?uncap_first} like '%${r'${query}'}%'</#if></#list> )
                <#else ><#--如果是字典Dict结尾，否则-->
                <#if field?exists>
                and ( <#list 0..(field!?size-1) as i><#if field[i].name?uncap_first != 'id'>t.${field[i].name?uncap_first} like '%${r'${query}'}%'<#if i!= field!?size-1> or </#if></#if></#list> )
                </#if>
                </#if>
            </if>
        </where>
