...
public class CodeGenerateUtils {
    ...
    /** 包的名称 ChenQi*/
    private final String PACKAGE_NAME = "org.ssh.boot.hisdata";
    /** 代码生成的位置 **/
...
/*    public Connection getConnection() throws Exception{
    ...
    public void generate() throws Exception{
        try {
             ...
             * description: 创建元数据集合
             * author: ChenQi <BR>
             * createDate: 2019-11-28 16:35  <BR>
             */
            List<Metadata> metadataList = generateMetadata();
            //4.生成实体类文件
            generateModelFile(metadataList);
            //5.生成Dao文件
    ...
    /**
     * methodName: genetateMetadata <BR>
     * description:创建元数据集合 <BR>
     * remark: <BR>
     * param:  <BR>
     * return: List<DBColumnDefinition> <BR>
     * author: ChenQi <BR>
     * createDate: 2019-11-28 16:39 <BR>
     */
    public List<Metadata> generateMetadata(){
        ...
        metadata.setColumnType("bigint");
        metadata.setChangeColumnName("Id");
        metadata.setColumnComment("主键");
        list.add(metadata);

        metadata = new Metadata();
        metadata.setColumnName("itemCode");
        metadata.setColumnType("varchar");
        metadata.setChangeColumnName("ItemCode");
        metadata.setColumnComment("编码");
        list.add(metadata);

        metadata = new Metadata();
        metadata.setColumnName("itemName");
        metadata.setColumnType("varchar");
        metadata.setChangeColumnName("ItemName");
        metadata.setColumnComment("名称");
        list.add(metadata);
        return list;
    }
     ...
     */
    private void generateModelFile(List<Metadata> metadataList) throws Exception{
        //文件的后缀
        final String suffix = ".java";
        //文件生成的路径
        final String path = DISK_PATH + TABLE_NAME + suffix;
        //文件的模板
        ...
        dataMap.put("model_column", metadataList);
